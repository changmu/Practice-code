...
static hash_node_t *hash_get_node_by_key(hash_t *hash, void *key, unsigned key_size);
...
hash_t *hash_alloc(unsigned buckets, hashfunc_t hash_func)
{
      ...
      hash->nodes = malloc(size);
      memset(hash->nodes, 0, sizeof(size));
...
{
        unsigned bucket = hash->hash_func(hash->buckets, key);
...
hash_node_t *hash_get_node_by_key(hash_t *hash, void *key, unsigned key_size)
{
        hash_node_t **bucket = hash_get_bucket(hash, key);
        ...
        while (node && memcmp(node->key, key, key_size)) {
                node = node->next;
        }
