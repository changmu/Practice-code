...
void shmfifo_put(shmfifo_t *fifo, const void *buf)
{
        ...
        memcpy(fifo->p_payload + fifo->p_shm->blksize * fifo->wr_index, buf, fifo->blksize);
        if (++fifo->wr_index == fifo->p_shm->blocks)
                fifo->wr_index = 0;
...
void shmfifo_get(shmfifo_t *fifo, char *buf)
{
        ...
        memcpy(buf, fifo->p_payload + fifo->p_shm->blksize * fifo->wr_index, fifo->blksize);
        if (++fifo->wr_index == fifo->p_shm->blocks)
                fifo->wr_index = 0;
